import { Component, Input, Output, EventEmitter, signal, computed, OnInit, ViewChild, ElementRef, HostListener } from '@angular/core';
    ...
    <div class="relative w-full">
      ...
      <div class="flex gap-2">
        ...
        <input
          ...
          [disabled]="disabled"
          class="flex-1 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
        />

        <!-- End Date Input (Range Mode) -->
        <input
          *ngIf="range"
          type="text"
          ...
          [attr.aria-label]="'End ' + (label || 'date')"
          [disabled]="disabled"
          class="flex-1 px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed"
        />
      </div>
      ...
      >
        ...
        <!-- Day Headers -->
        <div class="grid grid-cols-7 gap-1 px-4 mb-2">
          <div *ngFor="let day of dayHeaders" class="text-center text-xs font-semibold text-slate-600 dark:text-slate-400 py-2">
            {{ day }}
          </div>
        </div>
        ...
        <!-- Range Info -->
        <div *ngIf="range && selectedStartDate() && selectedEndDate()" class="px-4 pb-4 text-xs text-slate-600 dark:text-slate-400">
